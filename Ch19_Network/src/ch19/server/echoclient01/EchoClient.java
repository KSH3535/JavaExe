...
/* 클라이언트는 서버와 연결되어야 한다.
 * 1)연결 - IP, Port 를 소켓객체 설정하고 연결을 한다.
 * 2)통신 - 연결 후에 서버<->클라이언트 자유롭게 통신한다.
 * 3)종료 - 다 마치면 스트림을 종료한다.
 */
...
public class EchoClient {
	public static void main(String[] args) {
		...
		try {
			// IP, Port로 서버 프로그램과 연결 시도 후 성공하면(서버에서는 accept()가 리턴) 스트림 생성
			//그리고 연결된 socket 객체 리턴
			Socket socket = new Socket("192.168.0.205", PORT);	// 127.0.0.1은 내자신
