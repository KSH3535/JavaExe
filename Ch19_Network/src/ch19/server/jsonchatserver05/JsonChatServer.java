...
/*
 * [App 프로토콜 정의] 1) 데이터는 '문자열' 데이터로 한다. 2) '문자열' 데이터는 json을 변환한 것으로 한다. 3) json
 * 명령 패킷 종류 { }내의 데이터는 값을 의미 3-1) id 등록 [요청] cmd: ID id: {id값}
 * 
 * [응답] cmd: ID aclk: ok(성공), fail(실패)
 * 
 * 3-2) 사칙 연산 [요청] cmd: ARITH id: {id값} op: {연산자} val1: {첫번째값} val2: {두번째값}
 * 
 * [응답] cmd: ARITH ack: {결과값}
 * 
 * 3-3) 전체 채팅 [요청] cmd:: ALLCHAT id: {id값} msg: {문자메시지}
 * 
 * [응답] cmd: ID aclk: ok(성공), fail(실패)
 * 
 * [전송] cmd: BROADCHAT id: {id값} msg: {문자메시지}
 * 
 * 3-4) 1:1 채팅 [요청] cmd: ONECHAT id: {id값} yourid: {상대id} msg: {문자메시지}
 * 
 * [응답] cmd: ID aclk: ok(성공), fail(실패)
 * 
...
/*
 * JSON 라이브러리 다운로드 후 등록 1) https://github.com/stleary/JSON-java 2) Click here if
 * you just want the latest release jar file. 3) 프로젝트 폴더 내 lib 폴더 내에 복사 4) 해당
 * 프로젝트패키지 마우스오른쪽클릭 -> Build Path -> Add External Archives to Java Build Path
 * ->json.jar파일 선택 (import안될경우 module-info.java 삭제 혹은 설정)
 */
